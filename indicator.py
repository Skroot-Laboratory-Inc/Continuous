    ...
    def createIndicator(self, outerFrame):
        if self.totalNumberOfReaders > 1:
            ...
            if (self.readerNumber % 4) == 1:
                self.indicatorCanvas.place(anchor='e', relx=0.56, rely=0.15)
            if (self.readerNumber % 4) == 2:
                self.indicatorCanvas.place(anchor='e', relx=1.06, rely=0.15)
            if (self.readerNumber % 4) == 3:
                self.indicatorCanvas.place(anchor='e', relx=0.56, rely=0.6)
            if (self.readerNumber % 4) == 0:
                self.indicatorCanvas.place(anchor='e', relx=1.06, rely=0.6)
        else:
            ...
            self.indicator = self.indicatorCanvas.create_circle(27, 27, 25, fill=self.green, outline="black",
                                                                width=2)  # x, y, r, kwds
            self.indicatorCanvas.place(anchor='e', relx=1.05, rely=0.2)
